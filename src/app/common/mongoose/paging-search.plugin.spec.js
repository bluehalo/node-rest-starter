...
describe('Paging Search Plugin:', () => {
	describe('generateSort:', () => {
		...
		tests.forEach((test) => {
			it(test.description, () => {
				...
				should.exist(result);
				_.isEqual(result, test.expected).should.be.true(
			...
			it(test.description, () => {
				pagingSearchPlugin.__with__({
					pagingQuery: (schema, filter) => {
						_.isEqual(filter, test.expected).should.be.true(
							`expected filter ${JSON.stringify(
								filter
							)} to equal ${JSON.stringify(test.expected)}`
						);
					}
				})(() => {
					pagingSearchPlugin.__get__('searchContainsQuery')(
						null,
						test.input.query,
						test.input.fields,
						test.input.search
			...
			it(test.description, () => {
				pagingSearchPlugin.__with__({
					pagingQuery: (schema, filter, projection, options, sort) => {
						_.isEqual(filter, test.expected.filter).should.be.true(
							`expected filter ${JSON.stringify(
								filter
							)} to equal ${JSON.stringify(test.expected.filter)}`
						);
						_.isEqual(projection, test.expected.projection).should.be.true(
							`expected projection ${JSON.stringify(
								projection
							)} to equal ${JSON.stringify(test.expected.projection)}`
						);
						_.isEqual(sort, test.expected.sort).should.be.true(
							`expected sort ${JSON.stringify(sort)} to equal ${JSON.stringify(
								test.expected.sort
							)}`
						);
					}
				})(() => {
					pagingSearchPlugin.__get__('searchTextQuery')(
						null,
						test.input.query,
						test.input.search
					);
	...
	describe('plugin', () => {
		it('plugin should add static methods to schema', () => {
			...
			pagingSearchPlugin.__with__({
				searchTextQuery: () => {
					searchTextQueryCalled = true;
				},
				searchContainsQuery: () => {
					searchContainsQueryCalled = true;
				}
			})(() => {
			...
			searchTextQueryCalled.should.be.true(
				'expected searchTextQueryCalled to be called'
			);
			searchContainsQueryCalled.should.be.true(
				'expected searchContainsQueryCalled to be called'
			);
		});
	});
});
