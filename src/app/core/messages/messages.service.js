...
module.exports = function () {
	function getAllMessages() {
		...
		return Message.find({ created: { $gte: new Date(Date.now() - timeLimit) } })
			.lean()
			.exec();
	}
	...
	function getRecentMessages(userId) {
		return Promise.all([getAllMessages(), getDismissedMessages(userId)]).then(
			([messages, dismissedMessages]) => {
				messages = messages.filter(
					(message) =>
						!dismissedMessages.some((x) => {
		...
		const dismissals = messageIds.map((messageId) => {
			...
			return auditService
				.audit(
					...
					'message',
					'dismissed',
					TeamMember.auditCopy(user),
					Message.auditCopy(dismissedMessage),
					headers
				)
				.then(() => {
